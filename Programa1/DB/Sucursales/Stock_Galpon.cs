...
namespace Programa1.DB
{
    ...
    public class Stock_Galpon : c_Base
    {
        public Stock_Galpon()
        {
            Tabla = "dbGalpon.dbo.Ventas";
            Vista = "vw_Ventas";
            ID_Automatico = true;
        }
        ...
        {
            string s = "SELECT Id_Productos, Descripcion, SUM(Cantidad) AS Cantidad, SUM(Kilos) AS Kilos, (SUM(Costo_Entrada)/COUNT(Id_Productos)) as Precio FROM vw_Stock_Sis " +
                "WHERE fecha BETWEEN @f AND DATEADD(DAY, 6, @f) AND Id_Productos = 35 GROUP BY Id_Productos, Descripcion ";
            return Datos_Genericos(s);
        }

        public DataTable stock_Sis(string filtro = "")
        {
            if (filtro != "")
            {
                filtro = " WHERE " + filtro;
            }
            string s = $"SELECT Id_Productos, Descripcion, SUM(Cantidad) AS Cantidad, SUM(Kilos) AS Kilos, (SUM(Costo_Entrada)/COUNT(Id_Productos)) as Precio FROM vw_Stock_Sis {filtro} GROUP BY Id_Productos, Descripcion ORDER BY Id_Productos";

            return Datos_Genericos(s);
        }

        public DataTable stock_Sis_Cerdo(string filtro = "")
        {
            if (filtro != "")
            {
                filtro = " WHERE " + filtro;
            }
            string s = "SELECT id_Productos, Descripcion, ROUND(SUM(Kilos), 3) as Kilos, SUM(Cantidad) as Cantidad, Costo_Entrada FROM vw_Stock_Sis_cerdo" +
                $" {filtro} GROUP BY id_Productos, Descripcion, Costo_Entrada HAVING ROUND(SUM(Kilos), 3) > 0 ORDER BY Costo_Entrada";

            return Datos_Genericos(s);
        }

        public DataTable stock_Nico(string filtro = "")
        { 
            string s = $"SELECT id_Prod, Descripcion, SUM(Cantidad) AS Cantidad, SUM(Kilos) AS Kilos FROM vw_Galpon WHERE {filtro} GROUP BY Id_Prod, Descripcion ";
